以前のガイドラインに基づき、架空のTODOリストアプリ**「OneFocus」**を開発するための実例ドキュメント一式を作成しました。

このドキュメント群は、**「プロジェクトメンバーが実際に作成した（または作成すべき）成果物のゴールイメージ」**としてご活用ください。

---

# プロジェクト実例：一点集中型タスク管理「OneFocus」
...
*   ユーザーとして、思いついたタスクを高速で登録したい。
    ...
    *   **Out:** 期限日の設定、タグ付け、詳細メモ（これらは登録の邪魔になるので排除）。
...
*   ユーザーとして、**選んだタスク以外が見えない状態**になりたい。
    ...
    *   **Out:** ポモドーロタイマー（今回は「隠す」価値の検証が先なので不要）。
...
| **フォーカス** | `Focus` | **「今やっている」状態。これになれるのは常に1つのタスクのみ。** |
...
*   **Language:** TypeScript
*   **State Management:** React Context (単純なためRedux等は不要)
*   **Storage:** LocalStorage API (ブラウザ内保存)
*   **Styling:** Tailwind CSS (素早いUI構築のため)
